// Portado de apps/app/src/lib/validations/document.ts (Terceirizei OS) —
// mesmo algoritmo de dígito verificador de CPF/CNPJ, já testado em produção.
...
export function isValidCpfOnly(value: string): boolean {
  return isValidCpf(onlyDigits(value));
}

export function isValidCnpjOnly(value: string): boolean {
  return isValidCnpj(onlyDigits(value));
}

/** Só valida o formato (8 dígitos) — sem base oficial de CEP/endereço integrada. */
